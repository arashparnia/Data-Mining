...
rawdata <- read.csv("../../../Data/training_set_VU_DM_2014.csv", header= TRUE,nrows=20000)
```
...
# Transform Date time
```{r}
rawdata$date_time <-  anytime(as.factor(rawdata$date_time))
# data_raw$date_time
# tmp <-  as.Date('3/2/2015','%m/%d/%Y')
rawdata$year <- as.numeric(format(rawdata$date_time,'%Y'))
rawdata$month <- as.numeric(format(rawdata$date_time,'%m'))

rawdata$yearmonth <- rawdata$year + (rawdata$month / 12)
# 
```
...
```{r}
for (i in 1:34){                              # Does not work due to NA (instead of NULL)
