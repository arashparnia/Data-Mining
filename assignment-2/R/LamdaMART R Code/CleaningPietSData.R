...
Data_Pclass1<-Data_Pclass1[c(-2,-24,-25,-26,-27)]
Data_Pclass2<-Data_Pclass2[c(-2,-24,-25,-26,-27)]
Data_Pclass3<-Data_Pclass3[c(-2,-24,-25,-26,-27)]
Data_Pclass4<-Data_Pclass4[c(-2,-24,-25,-26,-27)]
Data_Pclass5<-Data_Pclass5[c(-2,-24,-25,-26,-27)]
Data_Pclass6<-Data_Pclass6[c(-2,-24,-25,-26,-27)]
##############################
# Form up final cleaned dataset
##############################
cd2<-rbind(Data_Pclass1,Data_Pclass2,Data_Pclass3,Data_Pclass4,Data_Pclass5,Data_Pclass6)
removals<-c("site_id","visitor_location_country_id","prop_country_id","position","srch_destination_id","srch_length_of_stay","srch_booking_window",
            "srch_adults_count","srch_children_count","srch_room_count","srch_saturday_night_bool")
cd2<-cd2[,!(colnames(cd2) %in% removals)]
norm_pr_usd<-cd2$normalized_prices
names(norm_pr_usd)[1]<-paste("norm_price_usd")
cd2<-cd2[c(-13)]
cd2<-cbind(cd2,norm_pr_usd)
save(cd2,file="CleanedData2.RData")
