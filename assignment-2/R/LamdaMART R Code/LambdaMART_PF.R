...
remove(consumer)
##############################
#LambdaMART training loop for each PClass (6 in total)
##############################
library("gbm")
for (i in seq(1,6,1) ){
Pname <- paste("gbm_Pclass", 2, sep = "_") 
gbm_ndcg_PC <- gbm(score~prop_starrating+prop_review_score+prop_brand_bool+prop_location_score1+prop_location_score2+prop_log_historical_price+price_usd+promotion_flag+random_bool+norm_price_usd, # formula
                data=cd[cd$Pclass==2,], # dataset
...
##############################
#LambdaMART training loop for promotion flag (4 in total)
##############################
gbm_ndcg_PF_1 <- gbm(score~prop_starrating+prop_review_score+prop_brand_bool+prop_location_score1+prop_location_score2+prop_log_historical_price+price_usd+promotion_flag+random_bool+norm_price_usd, # formula
                          data=cd[cd$promotion_flag==1,], # dataset
                          distribution=list(   # loss function:
                            name='pairwise',   # pairwise
                            metric="ndcg",     # ranking metric: normalized discounted cumulative gain
                            group='srch_id'),    # column indicating query groups
...
gbm_ndcg_PF_2 <- gbm(score~prop_starrating+prop_review_score+prop_brand_bool+prop_location_score1+prop_location_score2+prop_log_historical_price+price_usd+promotion_flag+random_bool+norm_price_usd, # formula
                        data=cd[cd$promotion_flag==0,], # dataset
                        distribution=list(   # loss function:
                          name='pairwise',   # pairwise
                          metric="ndcg",     # ranking metric: normalized discounted cumulative gain
                          group='srch_id'),    # column indicating query groups
