...
##############################  
for (i in seq(1,6,1) ){
    Pr_name <- paste("P_Prediction",i, sep = "_") 
    
    predictionTest<-data.frame(predict(get(paste("gbm_Pclass",i,sep="_")),cd_test[cd_test$Pclass==i,],
                                       gbm.perf(get(paste("gbm_Pclass",i,sep="_")),method='cv')))
    
    predictionTest<-cbind(cd_test$srch_id[cd_test$Pclass==i],predictionTest)
    names(predictionTest)[2]<-paste("P_Prediction",i,sep="_")
