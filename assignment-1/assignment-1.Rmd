...
# library(DAAG)
# cv.lm(data = ODI, fit1, m=3,printit = TRUE) # 3 fold cross-validation
cv.lm(data = ODI, form.lm=formula(age ~ chocolate + gender ),seed = 13, m=10,printit = TRUE)


# CVlm(data=ODI, form.lm=formula(log(age)~(chocolate)+(gender)),
#           plotit="Observed")
# CVlm(data=ODI, form.lm=formula(log(age)~(chocolate)+(gender)),
#      plotit="Residual")
# out <- CVlm(data=ODI, form.lm=formula(log(age)~(chocolate)+(gender)),
#                plotit="Observed")
# out[c("ms","df")]
...
#     m <- lm(formula = age ~  chocolate + gender  + money , data = trainData)
#     print(m)
