...
# Create the tree.
  output.tree <- ctree(
  gender ~  mach_lrn + stats +  db + info_ret + chocolate   ,  
  data = input.dat)
...
model <- lm(formula = age ~  chocolate + gender + program + money + bed_time, data = ODI)
# model <- glm (random ~ gender, data = ODI, family = binomial)
summary(model)
